   ...
   if (!empty($files)) {
      ...
      while (list($key, $file) = each($files)) {
         ...
         // File section
            ...
            if (($allowExtract) && ($subdir != $trashcan)) {
               echo "<a href=$thisscript?act=extract&subdir=".rawurlencode($subdir)."&sortby=$sortby&file=".str_replace("%2F", "/", rawurlencode(extractSubdir($file["path"])))." onMouseOver='return statusMsg(\"".quoteJS($messages["ext1"])."\");' onMouseOut='return statusMsg(\"\");'>".(($useimages) ? "<img src=\"$imagesdir/$downloadimage\" border=0>" : $messages["ext1"])."</a> ";
            }
