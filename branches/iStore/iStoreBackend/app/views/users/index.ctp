	...
	$(function(){
	    ...
	    grid = $('#tt2').datagrid({
			...
			columns:[[
				...
				{field:'username',title:'Name',width:120,sortable:true},
				{field:'passwd',title:'Passwd',width:120},
				{field:'addip',title:'AddIP',width:150},
				{field:'created',title:'CreateDate',width:150,rowspan:2,
...
//		            alert(data);
		...
		function edit() {
		    ...
		    if (num == 0) {
		        ...
		        win.window('open');
		        form.form('load', 'users/edit/' + rows[0].id);
	//	        #editform
		        form.url = 'users/edit';
		    }
		}
		...
		function del() {
		    ...
		    if (arr.length>0) {
		        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
		            if (data) {
		                $.ajax({
		                    url: 'users/delSelected/' + arr2str(arr),
		                    type: 'GET',
		                    error: function () {
		...
		function showAll() {
		    grid.datagrid({ url: 'users/listing' , queryParams: {} });
		}
		...
		function SearchOK() {
		    ...
		    grid.datagrid({ url: 'users/listing', queryParams: {name: s_name} });
		}
