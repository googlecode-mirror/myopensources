	...
	public String execute(String action, JSONArray args, String callbackId) {
		...
		try {
			if (action.equals("isAvailable")) {
				...
				return new PluginResult(status, b).getJSONString();
			}
				...
				return new PluginResult(status, i).getJSONString();
			}
				...
				return pluginResult.getJSONString();
			}			
			return new PluginResult(status, result).getJSONString();
